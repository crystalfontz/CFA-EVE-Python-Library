...
#enable for backlight sweep test
BACKLIGHT_TEST = True
...
class bounce_demo:
	...
	def add_to_display_list(self):
		...
		self.eve.buffer_cocmd_add(self.eve.defs.EVE_ENC_COLOR_RGB(self.r,self.g,self.b))
		#make it transparent
		self.eve.buffer_cocmd_add(self.eve.defs.EVE_ENC_COLOR_A(self.transparency))
		#draw the ball -- a point (filled circle)
		self.eve.cocmd.point(self.x_position, self.y_position, self.ball_size)
		...
		self.eve.buffer_cocmd_add(self.eve.defs.EVE_ENC_COLOR_RGB(200,0,0))
		#transparency set above still in effect
		self.eve.cocmd.line(self.eve.LCD_WIDTH >> 1, self.eve.LCD_HEIGHT >> 1, self.x_position >> 4, self.y_position >> 4, rubberband_width)
...
while True:
	...
	#backlight sweep test
	if BACKLIGHT_TEST == True:
		#do backlight sweep
		brightness += 1
		set_brightness = brightness
		if brightness < 128:
			#on the way up, nothing to do
			pass
		elif brightness < 256:
			#on the way down
			set_brightness = 128-(brightness-128)
		else:
			#reset to zero
			brightness = 0
			set_brightness = 0
		eve.set_backlight(set_brightness)
		if set_brightness % 10 == 0:
			print("brightness: {}".format(set_brightness))
